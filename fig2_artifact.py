...
Loads a model from a W&B artifact and generates mess3 belief visualization.
"""
...
def infer_config_from_checkpoint(checkpoint_path: Path) -> dict:
    ...
    n_ctx = state['pos_embed.W_pos'].shape[0]  # Use actual n_ctx from checkpoint
    ...
    print(f"Inferred config: {n_layers} layers, {d_model} dims, {n_heads} heads, vocab={d_vocab}, n_ctx={n_ctx}")
...
def generate_mm3_data(config: dict, device: str = 'cpu'):
    """Generate mess3 ground truth data.
    ...
    """
    print("Generating mess3 ground truth data...")

    # Get mm3 parameters from config
    process_config = config.get('process_config', {})

    # Extract mm3 params - handle both standard and cartesian formats
    if 'mm3' in process_config:
        mm3_params = process_config['mm3']
    else:
        mm3_params = {
            'x': process_config.get('x', 0.15),
            'a': process_config.get('a', 0.6),
        }

    # Create config for mm3 data generation
    mm3_config = {
        'process_config': {
            'name': 'mess3',
            **mm3_params
        ...
        'model_config': {
            **config['model_config'],
            'd_vocab': 3  # Always use 3 for mm3 data generation
        },
        'train_config': config['train_config'],
    }

    # Generate mm3 data
    nn_inputs, beliefs, indices, probs, _ = prepare_msp_data(mm3_config, mm3_config['model_config'])

    # If the actual model uses cartesian encoding (d_vocab > 3), remap tokens
    model_d_vocab = config['model_config'].get('d_vocab', 3)
    if model_d_vocab > 3:
        print(f"Remapping mm3 tokens for cartesian encoding (model d_vocab={model_d_vocab})...")
        # Cartesian encoding: mm3_token * vocab_bloch + bloch_token
        # We pair with bloch_token=0 to isolate mm3
        vocab_bloch = model_d_vocab // 3  # Should be 4 for mm3×bloch
        nn_inputs = nn_inputs * vocab_bloch  # Pair all with bloch=0
        print(f"  Using bloch context = 0 (vocab_bloch={vocab_bloch})")
...
def visualize_beliefs(gt_beliefs, pred_beliefs, weights, metrics, output_path):
    """Create visualization with ground truth and predictions.
    ...
    R = normalize_dim(gt_beliefs[:, 0])
    G = normalize_dim(gt_beliefs[:, 1])
    B = normalize_dim(gt_beliefs[:, 2])
    alpha = transform_for_alpha(weights, min_alpha=0.2)
    colors = np.stack([R, G, B, alpha], axis=-1)

    # Plot ground truth
    axes[0].scatter(x_gt, y_gt, color=colors, s=1.0, rasterized=True, marker='.')
    axes[0].set_title('Ground Truth', fontsize=14)
    axes[0].set_aspect('equal', adjustable='box')
    axes[0].set_axis_off()

    # Plot predictions
    axes[1].scatter(x_pred, y_pred, color=colors, s=1.0, rasterized=True, marker='.')
    ...
    # Add row label
    axes[0].text(-0.15, 0.5, 'Mess3', rotation=90, fontsize=14, ha='right',
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Generate mess3 visualization from W&B artifact or random init")
    ...
    parser.add_argument("--output", type=str, default="Figs/Fig2_artifact.png",
    ...
    print("=" * 80)
    if args.random_init:
        print("Mess3 Belief Visualization with Random Initialization")
    else:
        print("Mess3 Belief Visualization from W&B Artifact")
    print("=" * 80)
    ...
    nn_inputs, gt_beliefs, indices, probs = generate_mm3_data(config, device)

    # Use ALL timesteps for regression (not just last)
    ...
    # Step 5: Generate visualization (use last timestep for visualization)
